...
Процедура ЗаписатьРезультатПодключения(ИмяФайла, Результат)
	...
	ЗавершитьРаботуСистемы(ЗапрашиватьВозможностьЗавершенияРаботы);
...
Процедура ПодключитьЗагрузкуРасширения() Экспорт 
	...
	ЗапрашиватьВозможностьЗавершенияРаботы = Истина;
	...
	Если МассивПараметров.Количество()>2 Тогда
		...
		ЗапрашиватьВозможностьЗавершенияРаботы = Не НРег(МассивПараметров[2]) = "завершитьработусистемы";
...
Процедура ПроверитьДоступностьРазделенныхДанных()
	...
	ДоступноИспользованиеРазделенныхДанных = Ложь;
	Попытка
	
		МодульОбщегоНазначения = Вычислить("ОбщегоНазначенияПовтИсп");
		Если ТипЗнч(МодульОбщегоНазначения) = Тип("ОбщийМодуль") Тогда
			ДоступноИспользованиеРазделенныхДанных = МодульОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных();
		КонецЕсли;
...
Процедура ОбновитьПараметрыРаботыРасширений_БСП_2_4(ПроверяемыеРасширения = Неопределено, НеподключенныеРасширения = "")
	...
	ПараметрыВыполнения.Вставить("ТекстДляАсинхронногоВызова", "");
	ПараметрыПроцедуры = Новый Массив;
	ПараметрыПроцедуры.Добавить(ПараметрыВыполнения);
