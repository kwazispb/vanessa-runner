...
Процедура ЗагрузитьСписокФайлов(НайденныеФайлы)
	...
	Для каждого ЭлементМассива Из НайденныеФайлы Цикл
		...
		НачатьПомещениеФайла(Новый ОписаниеОповещения("ОкончаниеПомещенияВнешнейОбработки",
		ЭтотОбъект, ПараметрыРегистрации), , ИмяФайла, Ложь, УникальныйИдентификатор);
...
Процедура РезультатПоискаФайлов(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	КоличествоНеобработанныхФайлов = СостояниеПоискаФайлов.Количество() + СписокФайловДляЗагрузки.Количество();
	КонечныйАвтомат("ПроверитьКоличествоФайловКЗагрузкеИЗавершить", КоличествоНеобработанныхФайлов);
...
Процедура РекурсивнаяЗагрузкаВнешнихОбработок(ДополнительныеПараметры)
	...
	Для каждого Элемент Из СписокМасокФайлов Цикл
		...
		НачатьПоискФайлов(Новый ОписаниеОповещения("РезультатПоискаФайлов", ЭтотОбъект, Контекст, 
		"ОшибкаПоискаФайлов", ЭтотОбъект), Контекст.Файл, Контекст.МаскаПоиска, Истина);
